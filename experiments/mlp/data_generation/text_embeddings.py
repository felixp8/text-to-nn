...
instruction = "Represent the Math equation: " # it's actually an expression, or maybe function?
...
for i in range(0, base_csv.shape[0], batch_size):
    expr_list = base_csv['expr'].iloc[i:i+batch_size]
    expr_list = [clean_expr(expr) for expr in expr_list]
    [validate_expr(expr) for expr in expr_list]

    input_list = [
        [instruction, expr] for expr in expr_list
    ]

    batch_embeddings = model.encode(input_list)
